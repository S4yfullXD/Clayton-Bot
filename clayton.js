...
class Clayton {
  ...
  loadWallets() {
    try {
      ...
      this.log(`An error occurred while loading wallet data: ${error.message}`, "error");
      return [];
    }
  }
    ...
    while (fetchAttempts < maxAttempts) {
      ...
      if (tasksResult.success) {
        ...
        for (const task of uncompletedTasks) {
          this.log(`Starting the Mission ${task.task_id} | ${task.task.title}...`);
...
async function main() {
  console.log(colors.yellow("(https://t.me/Sayfull_XD)"));
